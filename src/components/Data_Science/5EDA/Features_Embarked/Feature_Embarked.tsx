...
import Detaildataclean from "./detail_Embarked";
...
export default function DataCleaning({}: Props) {
  ...
  const childToParent = (childdata: any, id: number) => {
    ...
    if (childdata.option === "frequency port") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# หาว่าท่าเรือไหนที่ผู้โดยสารใช้งานสูงสุด
              \nfreq_port = train_data.Embarked.dropna().mode()[0]
              \nfreq_port`,
      }));
    } else if (childdata.option === "clean missing with freq") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# clean missing value with ท่าเรือที่ผู้โดยสารใช้งานสูงสุด
      ...
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# convert feature from S,C,Q ----> 0,1,2
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8 place-items-stretch mt-2 h-fit w-auto p-2">
        <div className="relative h-auto min-w-[310px] w-full p-5 rounded-lg bg-gradient-to-r from-gray-800 to-gray-800 border-1 border-black">
          <h1 className="text-xl text-white">Feature Embarked</h1>
