...
import Detaildataclean from "./detail_Alone";
...
export default function DataCleaning({}: Props) {
  ...
  const childToParent = (childdata: any, id: number) => {
    ...
    if (childdata.option === "visualization SiSp") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# ดูความสัมพันธ์ของจำนวน พี่ น้อง แฟน ภรรยา กับอัตรการรอดชีวิต
              \ntrain_data[["SibSp", "Survived"]].groupby(['SibSp'], \\
              \nas_index=False).mean().sort_values(by='Survived', ascending=False)`,
      }));
    } else if (childdata.option === "visualization Parch") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# ดูความสัมพันธ์ของจำนวน พ่อ แม่ ลูก กับอัตรการรอดชีวิต
              \ntrain_data[["Parch", "Survived"]].groupby(['Parch'], \\
              \nas_index=False).mean().sort_values(by='Survived', ascending=False)`,
      }));
    } else if (childdata.option === "New feature Alone") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# New feature ถ้าขนาด Family เป็น 1 คนจะให้เป็น Alone
              \ntrain_data['Alone'] = 0
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8 place-items-stretch mt-2 h-fit w-auto p-2">
        <div className="relative h-auto min-w-[310px] w-full p-5 rounded-lg bg-gradient-to-r from-gray-800 to-gray-800 border-1 border-black">
          <h1 className="text-xl text-white">Feature Alone</h1>
