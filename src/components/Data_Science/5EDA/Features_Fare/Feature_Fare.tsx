...
import Detaildataclean from "./detail_Fare";
...
export default function DataCleaning({}: Props) {
  ...
  const childToParent = (childdata: any, id: number) => {
    ...
    if (childdata.option === "clean missing value") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# Cleaning Age null value to mean
    ...
    } else if (childdata.option === "FareBand") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# แบ่งราคาของตั๋ว ออกเป็น 4 ช่วงราคา และ visualization table
      ...
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# convert feature from range Fare to number categories
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8 place-items-stretch mt-2 h-fit w-auto p-2">
        <div className="relative h-auto min-w-[310px] w-full p-5 rounded-lg bg-gradient-to-r from-gray-800 to-gray-800 border-1 border-black">
          <h1 className="text-xl text-white">Feature Fare</h1>
