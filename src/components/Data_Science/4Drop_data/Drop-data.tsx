...
import Detaildataclean from "./detail_drop";
...
export default function DataCleaning({}: Props) {
  ...
  const childToParent = (childdata: any, id: number) => {
    ...
    if (childdata.option === "Drop columns") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `train_data = train_data.drop(['PassengerId','Ticket', 'Cabin','Name'], axis=1)
            \ntrain_data`,
      }));
    } else if (childdata.option === "describe") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `
            \ntrain_data.describe()`,
  ...
  return (
    <>
      <div className="flex flex-row md:flex-cols align-middle h-auto w-auto p-2 rounded-lg bg-white mt-1">
        <div className="w-auto h-fit  p-0 ml-5 flex-none">Details data </div>
