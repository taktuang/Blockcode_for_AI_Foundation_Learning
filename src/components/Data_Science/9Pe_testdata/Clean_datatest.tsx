...
import Detaildataclean from "./detail_pretest";
...
export default function DataCleaning({}: Props) {
  ...
  const childToParent = (childdata: any, id: number) => {
    ...
    if (childdata.option === "read test data") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# read data from csv
              \ntest_data = pd.read_csv('./titanic/test.csv')       `,
      }));
    } else if (childdata.option === "preprocessing data") {
      setFormData((prevData) => ({
        ...prevData,
        [id]: `# accuracy plot
        \n# drop data ที่ไม่ใช้
        \ntest_data = test_data.drop(['Ticket', 'Cabin','Name'], axis=1)
        \n# convert feature to 0, 1
        \ntest_data['Sex'] = test_data['Sex'].map( {'female': 1, 'male': 0} ).astype(int)
        \n# clean and convert feature Age
        \nprint(f"Median Passenger Age: {median_age}")
        ...
        \n# New feature Alone
        \ntest_data['Alone'] = 0
        \ntest_data.loc[test_data['FamilySize'] == 1, 'Alone'] = 1
        \n# clean and convert feature Embarked
        \ntest_data['Embarked'] = test_data['Embarked'].fillna(freq_port)
        \ntest_data['Embarked'] = test_data['Embarked'].map( {'S': 0, 'C': 1, 'Q': 2} ).astype(int)
        \n# clean and convert feature Fare
        \nprint(f"Median Passenger Fare: {median_fare}")
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8 place-items-stretch mt-2 h-fit w-auto p-2">
        <div className="relative h-auto min-w-[310px] w-full p-5 rounded-lg bg-gradient-to-r from-gray-800 to-gray-800 border-1 border-black">
          <h1 className="text-xl text-white">preprocess ข้อมูลชุด test</h1>
  ...
  return (
    <>
      <div className="flex flex-row md:flex-cols align-middle h-auto w-auto p-2 rounded-lg bg-white mt-1">
        <div className="w-auto h-fit  p-0 ml-5 flex-none">preprocessing data</div>
